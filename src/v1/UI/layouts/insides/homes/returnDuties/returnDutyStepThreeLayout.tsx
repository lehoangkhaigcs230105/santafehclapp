import styles from "src/assets/styles/tabStyles/homeStyles/returnDutyStyles/returnDutyStepThree.styles";
...
const ReturnDutyStepThreeLayout = () => {
  ...
  useEffect(() => {
    const loadPrefill = async () => {
      try {
        ...
        console.error("Return duty step 3 prefill error:", error);
      }
    };
  ...
  const pushToReturnDutyFinalStep = () => {
    if (!driverFirstName.trim() || !driverLastName.trim() || !driverPhone.trim()) {
      Alert.alert("Error", "Please complete the driver information");
      return;
    }

    navigation.navigate(StackScreens.returnDutyFinalStep);
  };

  return (
    <View style={styles.container}>
      <HeaderTabs title="Return Of Duty Step 3" showBack />
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        <View style={styles.form}>
          ...
          <ButtonForm onPress={pushToReturnDutyFinalStep} text={t("next")} />
        </View>
      </ScrollView>
    </View>
  );
...
export default ReturnDutyStepThreeLayout;
